...
echo "Delete last current snapshot"
curl $AUTH -X DELETE "localhost:${PORT}/_snapshot/${ELASTIC_INDEX}/current?pretty"

echo "Create last current snapshot"
curl $AUTH -X PUT \
"localhost:${PORT}/_snapshot/${ELASTIC_INDEX}/current?wait_for_completion=true&pretty" \
-H 'Content-Type: application/json' -d'
{
  "indices": "'${ELASTIC_INDEX}'",
  "ignore_unavailable": true,
  ...
  "metadata": {
    "taken_by": "ikanashov",
    "taken_because": "last current snapshot"
  }
}
'